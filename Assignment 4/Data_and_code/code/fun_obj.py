...
class LogisticRegressionLossL2(LogisticRegressionLoss):
    ...
    def evaluate(self, w, X, y):
        ...
        # Calculate the function value
        f = np.sum(np.log(1 + np.exp(-yXw)))+self.lammy/2*w_L2
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        for i in range(n):
            left = -W[y[i]].T @ X[i]
            right = np.log(np.sum(np.exp(W @ X[i])))
            f = f+ left+right
        ...
        for c in range(k):
            for j in range(d):
                for i in range(n):
                    sum = np.sum(np.exp(W @ X[i]))
                    left = -SoftmaxLoss.indicator(i, c, y)*X[i, j]
                    right = X[i, j] * np.exp(W[c].T @ X[i]) / sum
                    g[c, j] = g[c, j] + left + right
