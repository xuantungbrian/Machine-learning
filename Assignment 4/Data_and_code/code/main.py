...
def q1_1():
    ...
    X, y = data["X"], data["y"]
    X_test, y_test = data["Xvalid"], data["yvalid"]
    model = linear_models.RegularizedRBF(1, 10**(-12))
    model.fit(X, y)
    # Compute the error on the validation set
    yhat = model.predict(X_test)
    validError = np.mean((yhat - y_test)**2)
    print("With sigma = {:.3f}, validError = {:.2f}".format(1, validError))
...
def q1_2():
    ...
    for sigma in [2.0**i for i in range(-15, 16)]:
        ...
        for j in range (10):
            # Train on the training set
            mask = np.ones(n)
            mask[j*ten:(j+1)*ten] = False
            X_valid = X[mask==False,:]
            y_valid = y[mask==False,:]
            X_train = X[mask==True, :]
            y_train = y[mask==True, :]            
            model = linear_models.RegularizedRBF(sigma, 10**(-12))
            model.fit(X_train, y_train)
            # Compute the error on the validation set
            yhat = model.predict(X_valid)
            validError = np.mean((yhat - y_valid)**2)
            totalValid = totalValid+validError

        validError = totalValid/10
        print("With sigma = {:.3f}, validError = {:.2f}".format(sigma, validError))
    ...
    # Now fit the model based on the full dataset
    model= linear_models.RegularizedRBF(bestSigma, 10**(-12))
    model.fit(X, y)
    # Report the error on the test set
    yhat = model.predict(X_test)
    testError = np.mean((yhat - y_test)**2)
