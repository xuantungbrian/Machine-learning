...
class RegularizedRBF:
    ...
    def fit(self, X, y):
        ...
        n,d = Z.shape
        self.X_fit_ = X
        self.w = solve(Z.T @ Z + self.lambd*np.identity(n), Z.T @ y)
    ...
    def fit(self, X, y):
        ...
        while not np.all(selected):
            ...
            for j in range(d):
                ...
                X_new = X[:,selected_with_j == True]

                #calculate w
                model = LinearClassifier(self.loss_fn, self.optimizer)
                model.fit(X_new,y)

                #calculate loss
                loss, grad = self.global_loss_fn.evaluate(model.w, X_new, y)
                if (min_loss > loss):
                    min_loss = loss
                    best_feature = j
...
class MulticlassLinearClassifier(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        W = np.reshape(self.w, (k, d))
        self.W = W

    def predict(self, X_hat):
        y_hat = X_hat @ self.W.T
